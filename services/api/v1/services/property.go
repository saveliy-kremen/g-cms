...
func (u *PropertyServiceImpl) Properties(ctx context.Context, req *empty.Empty) (*v1.PropertiesResponse, error) {
	user_id := auth.GetUserUID(ctx)

	properties := []models.Property{}

	db.DB.Where("user_id = ?", user_id).Order("sort").Find(&properties)
